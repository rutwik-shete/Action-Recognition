...
class CustomModel(nn.Module):
    def __init__(self):
        ...
        # nn.Sequential(
            ...
            # nn.TransformerEncoderLayer(d_model=25088,nhead=14,dim_feedforward = 512,activation = 'relu'),
            # nn.Flatten(),
            # BatchFlattening(),
            # nn.Linear(2032128,len(CATEGORY_INDEX),bias=True)
        # )
        self.midLayer = nn.Sequential(
            PositionalEncoding(d_model=512,batchSize=self.batchSize),
            nn.TransformerEncoderLayer(d_model=512,nhead=16,dim_feedforward = 512,activation = 'relu'),
            nn.Flatten(),
            # BatchFlattening(),
